...
function isBlockingDependency(dependency) {
  return (
    !dependency.type || ["blocks", "conditional-blocks", "waits-for"].includes(dependency.type)
  );
}
...
function buildWasmGraph() {
  ...
  try {
    ...
    // Add blocking edges
    store.dependencies.filter(isBlockingDependency).forEach((d) => {
...
function validateLayout(layout, issues, dependencies) {
  ...
  if (
    ...
    ids.size !== issues.length ||
    layout.node_count !== ids.size ||
    Object.keys(positions).length !== ids.size
  )
    return null;
  for (const id of ids) {
  ...
  const edges = dependencies
    ...
    .map((dep) => JSON.stringify([dep.depends_on_id, dep.issue_id]))
    .sort();
  if (
    !Array.isArray(layout.links) ||
    layout.edge_count !== edges.length ||
    layout.links.length !== edges.length
  )
    return null;
  const links = [];
  for (const link of layout.links) {
    if (!Array.isArray(link) || link.length !== 2 || !link.every((id) => typeof id === "string"))
      return null;
  ...
  if (snapshot?.version === 1) {
    for (const [name, statusName] of [
      ...
      if (
        ...
        Object.keys(values).length !== nodeIDs.size
      )
        continue;
      ...
      if (
        name === "pagerank" &&
        nodeIDs.size &&
        Math.abs(Object.values(values).reduce((sum, n) => sum + n, 0) - 1) > 0.0001
      )
        continue;
  ...
  if (store.wasmReady) {
    ...
    if (store.wasmGraph && centrality) {
      for (const name of ["pagerank", "betweenness"]) {
        if (centrality[name]) {
          store.metrics[name] = Array.from(
            { length: store.wasmGraph.nodeCount() },
            (_, i) => centrality[name][store.wasmGraph.nodeId(i)],
          );
...
function getNodeColor(node) {
  ...
  if (node.blockerCount > 0 && (node.status === "open" || node.status === "in_progress")) {
...
function timelineOpacity(state, now = performance.now()) {
  const progress = state.duration
    ? Math.min(1, Math.max(0, (now - state.startedAt) / state.duration))
    : 1;
...
function redrawTimelineTransition() {
  ...
  const pending = [...timeTravelState.nodeStates.values()].some(
    (state) => now < state.startedAt + state.duration,
  );
  if (!timeTravelState.active) {
...
function createTimelineControls() {
  ...
  const dated = commits
    .map((commit, idx) => ({ idx, at: Date.parse(commit.date) }))
